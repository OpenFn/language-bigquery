...
>>>>>>> 3d9663b70defabf58d165ec5918a53a9b5eb63e7
  ...
  return function (state) {
    ...
    return new Promise(function (resolve, reject) {
      (0, _request["default"])({
        ...
        //console.log(body);
        resolve(body);
        var zip = new _admZip["default"](body);
...
function load(fileName, datasetId, tableId, loadOptions, bQOptions, callback) {
  // something that loads data (from a CSV?) to BigQuery
  return function (state) {
    var bigquery = new BigQuery(bQOptions); // In this example, the existing table contains only the 'Name', 'Age',
=======
function unzip(pathToCsv) {// something that unzips from a CSV and allows the output to be used for the
  // input of `load(data, options)`
}
